...
string *reg ::i_placer(std::string binary) {
    string *temp = new string[5];
    temp[0] = binary_split(binary , 0, 9); //10 bits opcode
    temp[1] = binary_split(binary , 10 , 21); //12 bits address
    temp[2] = binary_split(binary , 22 , 26); //5 bits Rn
    temp[3] = binary_split(binary , 27 , 31); //5 bits Rd
    return temp;
}
...
void reg :: r_display(string *array , ofstream& file){
    ...
    file << "----------------------------" << endl;
    file << "R-FORMAT : " << endl;
    file << "opcode : " << array[0] << "  (" << stoull(array[0] , 0, 2) << ")" << endl;
    file << "Rm : " << array[1] << "  (" << stoull(array[1] , 0, 2) << ")" << endl;
    file << "Shamt :" << array[2] << "  (" << stoull(array[2] , 0, 2) << ")" << endl;
    file << "Rn : " << array[3] << "  (" << stoull(array[3] , 0, 2) << ")" << endl;
    file << "Rd :" << array[4] << "  (" << stoull(array[4] , 0, 2) << ")" << endl;
    file << "----------------------------" << endl;
...
void reg :: d_display(string *array , ofstream& file){
    ...
    file << "D-FORMAT :" << endl;
    file << "opcode : " << array[0] << "  (" << stoull(array[0] , 0, 2) << ")" << endl;
    file << "address : " << array[1] << "  (" << stoll(array[1] , 0, 2) << ")" << endl;
    file << "op :" << array[2] << "  (" << stoull(array[2] , 0, 2) << ")" << endl;
    file << "Rn : " << array[3] << "  (" << stoull(array[3] , 0, 2) << ")" << endl;
    file << "Rt :" << array[4] << "  (" << stoull(array[4] , 0, 2) << ")" << endl;
    file << "----------------------------" << endl;
...
void reg :: cb_display(string *array , ofstream& file){
    ...
    file << "opcode : " << array[0] << "  (" << stoull(array[0] , 0, 2) << ")" << endl;
    file << "address : " << array[1] << "  (" << stoll(array[1] ,0 , 2) << ")" << endl;
    file << "Rt : " << array[2] << "  (" << stoull(array[2] , 0, 2) << ")" << endl;
    file << "----------------------------" << endl;
}

void reg :: i_display(string *array , ofstream &file){

    file << "----------------------------" << endl;
    file << "I-FORMAT :" << endl;
    file << "opcode : " << array[0] << "  (" << stoull(array[0] , 0, 2) << ")" << endl;
    file << "ALU Immediate : " << array[1] << "  (" << stoll(array[1] , 0, 2) << ")" << endl;
    file << "Rn : " << array[2] << "  (" << stoull(array[2] , 0, 2) << ")" << endl;
    file << "Rd : " << array[3] << "  (" << stoull(array[3] , 0, 2) << ")" << endl;
    file << "----------------------------" << endl;
...
void reg::print_register_content(ofstream& file){
    int i =0;
    while(i<32){
        file << "Register " << i << " : " << bitset<64>(regist[i]) << "  (" << regist[i] << ")" << endl;
        i++;
    }
}


void reg::display_register_content(){

    int i =0;
    while(i<32){
        cout << "Register " << i << " : " << bitset<64>(regist[i]) << "  (" << regist[i] << ")" << endl;
        i++;
    }
